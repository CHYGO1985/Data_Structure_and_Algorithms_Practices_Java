...
public class FourSum {
    ...
    public static List<Integer> add(int i,int j,int k,int m){
    ...
    public static List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        // method 3: same idea as 2, but with different strategy to compare Map
        int[] num = nums;
        Arrays.sort(num);
        int len = num.length;
                ...
                if(map.containsKey(targetMinus)){
                    ...
                    for(int p = 0;p < list.size();p ++){
                        ...
                        if(flag&&res.get(res.size()-1).get(2)==num[temp[0]])
                            continue;
                        List<Integer> l=add(i,j,temp[0],temp[1]);
                        res.add(l);
                        flag=true;
                    }
                }
